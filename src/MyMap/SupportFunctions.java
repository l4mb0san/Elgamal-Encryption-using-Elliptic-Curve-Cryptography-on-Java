...
public class SupportFunctions {
    ...
    /*generating key pair [Point publickey, BigInteger privatekey]*/
    /*inputexcelFilePath is generating sets of points from a point path*/
    /*n is the maximum number of point in inputexcelFilePath*/
    public KeyPair keypairGeneration(BigInteger n, String inputexcelFilePath) {
        ...
        //1. Select d belong [1, n-1]
            d = obj.randBigInt(n);//rand(1, n-1);
        //2. Compute Q = dP
            Q.kPoint(d, n, inputexcelFilePath);
...
//                System.out.println(cnt + " Using next prime " + i);
    ...
    public void tablePointDoubling(EllipticCurve E, Point P, BigInteger times, String tableSaveFilePath) {
        ...
        try (FileOutputStream outputStream = new FileOutputStream(tableSaveFilePath)) {
            Row row = sheet.createRow(rowCount);
            for(int i = 0; i < lastValue; i++) {
                point = point.Pointdoubling(E);
                HandleFile.writeExcel(row, rowCount, columnCount++, point); //2P, 4P, 8P, ...
